...
import com.fling.tools.mapper_sql.MapperSqlIconProvider;
...
public class CodingGuidelinesHelper {
            ...
            public void run() {
                ...
                if (presetFile == null || !presetFile.exists()) {
                    ...
                    try {
                        ...
                        FlingHelper.notify(project, NotificationType.ERROR, "Failed to create coding-guidelines config file: pre-set.json");
                    }
                }else{
                    try (BufferedReader reader = new BufferedReader(new InputStreamReader(presetFile.getInputStream(), StandardCharsets.UTF_8))) {
                        ...
                        FlingHelper.notify(project, NotificationType.ERROR, "Failed to modify coding-guidelines config file: pre-set.json");
                    }
                }
                ...
                for (DocSource docSource : DocSource.values()) {
                    ...
                    if (targetFile == null || !targetFile.exists()) {
                        ...
                        try {
                            ...
                            FlingHelper.notify(project, NotificationType.ERROR, "Failed to create coding-guidelines config file: " + docSource.name().replace("_", "-"));
                        }
                    }
                }
            }
    ...
    public static void refreshPresetDoc(Project project) {
        ...
        try {
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(targetFile.getInputStream(), StandardCharsets.UTF_8))) {
                ...
                FlingHelper.notify(project, NotificationType.ERROR, "Fail load coding-guidelines, source:pre-set.json , " + e.getClass().getSimpleName() + ", " + e.getMessage());
            }
        } finally {
    ...
    public static Map<String, Doc> refreshDoc(Project project, DocSource strategy) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(targetFile.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            FlingHelper.notify(project, NotificationType.ERROR, "Fail load coding-guidelines, source:" + strategy.name().replace("_", "-") + ".json , " + e.getClass().getSimpleName() + ", " + e.getMessage());
            return null;
        }
    }
