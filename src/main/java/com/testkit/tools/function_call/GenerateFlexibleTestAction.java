...
public class GenerateFlexibleTestAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        if (module == null) {
            TestkitHelper.notify(project, NotificationType.ERROR, "Cannot find module for current file");
            return;
        }
    ...
    private void generateAndOpenTestClass(Project project, Module module, String className, String content) {
        ...
        VirtualFile testSourceRoot = FunctionCallIconProvider.getTestSourceRoot(module);
        if (testSourceRoot == null) {
            // 如果没有找到，尝试创建目录
            try {
                testSourceRoot = FunctionCallIconProvider.createTestRoot(module);
            } catch (Throwable e) {
    ...
    public void generateOrAddTestMethod(Project project, PsiDirectory testSourceRoot, String className, String content) {
        ...
        WriteCommandAction.runWriteCommandAction(project, new Runnable() {
            @Override
            public void run() {
                PsiClass testClass = FunctionCallIconProvider.findTestClass(testClassName, packageName, project);
                if (testClass == null) {
                    String classContent = generateTestClassContent(packageName, className);
                    PsiDirectory packageDir = FunctionCallIconProvider.createPackageDirectory(project, testSourceRoot);
                    PsiJavaFile testFile = FunctionCallIconProvider.createTestFile(packageDir, testClassName, classContent);
                ...
                PsiMethod testMethod = FunctionCallIconProvider.findTestMethod(testClass, methodName);
                if (testMethod == null) {
                    testClass.navigate(true);
                } else {
                    testMethod.navigate(true);
                }
                TestkitHelper.copyToClipboard(project, content, "The selected code has been copied");
            }
        });
